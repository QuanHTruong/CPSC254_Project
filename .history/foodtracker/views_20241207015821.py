...
def food_list_view(request):
    ...
    foods = Food.objects.all()
     ...
     # Calculate expiration details for each food item
    food_list = []
    for food in foods:
        food.image = food.get_images.first()
         # Calculate days until expiration
        if food.expiration_date:
            days_until_expiration = (food.expiration_date - date.today()).days
            progress = max(0, 100 - ((days_until_expiration / 30.0) * 100))  # Progress bar percentage
        else:
            days_until_expiration = None
            progress = None  # No expiration date provided
